...
function generate_invoice_pdf($invoice_id, $stream = true, $invoice_template = null, $is_guest = null)
{
    ...
    // 🔽 START: PDF name logic
    $pdf_prefix = ($invoice->invoice_balance == 0) ? 'Receipt' : trans('invoice');
    $invoice_number_clean = str_replace(array('\\', '/'), '_', $invoice->invoice_number);
    $pdf_name = $pdf_prefix . '_' . $invoice_number_clean;
    // 🔼 END: PDF name logic

    return pdf_create($html, $pdf_name, $stream, $invoice->invoice_password, true, $is_guest, $include_zugferd, $associatedFiles);
}
